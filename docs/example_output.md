...
`docs/architecture.md` (Known Limitations → Consensus Summary) for why.
...
GenLayer Studio, with the on-chain context grounding feature (see
...
**Input:** `"Proposal to increase treasury spending by 10% to fund additional grants over the next two quarters."`
...
above, the contract exposes several read-only views built from the same
...
- **`get_report_markdown(simulation_id)`** — renders the same report as
  readable Markdown instead of JSON. No additional LLM call is performed.

- **`get_normalizer_diff(simulation_id)`** — shows how many scenarios the
  raw LLM output contained before the Normalizer merged or dropped any of
  them, which titles did not make it into the final report, and the full
  lists of `raw_titles` vs `final_titles`.
...
- **`get_confidence_trend(category)`** and **`get_category_stats()`** —
  give running totals across all simulations of a category, rather than a
  single simulation's detail.

- **`get_onchain_context(simulation_id)`** — returns the JSON context
  actually used for that specific simulation (empty string if none was
  used), distinct from `get_onchain_context_config()`, which returns the
  contract-wide feature toggle and configured data source URL rather than
  anything tied to one simulation.

- **`get_proposal(simulation_id)`** — returns the original, unmodified
  proposal text submitted to the simulation.

- **`list_recent_simulations(limit)`** — returns a compact list of recent
  simulations (newest first) with id, proposal summary, and detected type,
  letting a frontend build a history view without fetching every full
  report individually.

- **`find_similar_simulations(category, limit)`** — returns recent
  simulations that match the given category (bounded to the most recent
  1000 entries to keep scan cost predictable).
